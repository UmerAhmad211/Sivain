...
let rec emit_exp ?(dt = "w") ids_map (ids_map_func : (string, string) Hashtbl.t)
    env = function
  | Int (_, i) ->
      let t = make_tmp () in
      emit "%s =w copy %d\n" t i;
...
let rec emit_stmts ids_map ids_map_func env = function
  ...
  | PDecl (name, dt, eopt) -> (
      let t = make_tmp () in
      (* altho this match is not necessary but i still should use
       it incase of sema bugs *)
      match Vars.find env name with
...
let emit_func ids_map ids_map_func f =
  ...
  List.iter
    (fun (name, ty) ->
      let align = ret_align_size ty in
      let t = make_tmp () in
      emit "%s =l alloc%d %d\n" t align align;
      let ret = ret_dtype ty in
      emit "store%s %s, %s\n" ret (Hashtbl.find func_local_map name) t;
      Hashtbl.replace func_local_map name t)
    f.pparams;
  List.iter
