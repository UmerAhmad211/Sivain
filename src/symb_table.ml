...
module Vars = struct
  type t = (string, dtype * pos) Hashtbl.t list

  let create () : t = [ Hashtbl.create 4 ]
  let push_scope (env : t) : t = Hashtbl.create 4 :: env

  let pop_scope (env : t) : (t, string) result =
    match env with
    | [] -> Error "No scope to pop."
    | _ :: rem -> Ok rem

  let add (env : t) (name : string) (dt : dtype) (p : pos) =
    match env with
    ...
    | scope :: _ ->
        if Hashtbl.mem scope name then
          ...
          Ok (Hashtbl.add scope name (dt, p))
...
module Funcs = struct
  type t = (string, dtype * pos * (string * dtype) list) Hashtbl.t

  let create () : t = Hashtbl.create 4

  let add (env : t) (name : string) (rdt : dtype) (p : pos)
      (params : (string * dtype) list) =
    if Hashtbl.mem env name then
      Error ("Duplicate Function name " ^ name)
    else
      Ok (Hashtbl.add env name (rdt, p, params))

  let find (env : t) (name : string) : dtype * pos * (string * dtype) list =
